...
------------------------------------------------------------
local os_clock, type, pairs, tonumber, tostring = os.clock, type, pairs, tonumber, tostring
local math_sqrt, math_max, math_min, math_floor, math_abs = math.sqrt, math.max, math.min, math.floor, math.abs
local bit_band, bit_bor, str_format = bit.band, bit.bor, string.format
...
------------------------------------------------------------
local cast_state = {
    name          = '', target = '', target_color = {1,1,1,1}, target_idx = 0,
    is_item       = false, last_pct = 0, last_tick = 0, queued_time = 0,
...
ashita.events.register('packet_out', 'targetbar_packet_out', function(e)
    ...
    local target_idx, action_name, is_item = 0, '', false
    ...
    if e.id == 0x1A then
        ...
        elseif category == 7 or category == 9 then
            ...
            if res and res.Name then action_name = res.Name[1] or res.Name[0] else action_name = 'ITEM'; is_item = true end
        elseif category == 5 then action_name = 'ITEM'; is_item = true
        else
    ...
    elseif e.id == 0x37 then
        target_idx = struct.unpack('H', e.data_modified, 0x08 + 1)
        action_name = 'ITEM'; is_item = true
    end

    if action_name == '' or action_name == 'Gil' then return end

    local entity = mm:GetEntity()
    local target_name, target_color = 'Self', COLOR_PC_SELF
    ...
    cast_state.name, cast_state.target, cast_state.target_color, cast_state.target_idx, cast_state.is_item, cast_state.queued_time = 
    ...
    entry.name, entry.target, entry.target_color, entry.target_idx, entry.is_item, entry.time = 
        cast_state.name, cast_state.target, cast_state.target_color, cast_state.target_idx, cast_state.is_item, cast_state.queued_time
end)
...
ashita.events.register('packet_in', 'targetbar_packet_in', function(e)
    ...
    if msg_id == 16 or msg_id == 29 then
        cast_state.name = ''; cast_state.queued_time = 0; history_count = 0
    elseif REJECT_MSGS[msg_id] then
        ...
        if history_count > 0 then
            ...
            if (os_clock() - prev.time) < 15.0 and get_cast_pct() > 0 then
                cast_state.name, cast_state.target, cast_state.target_color, cast_state.target_idx, cast_state.is_item, cast_state.queued_time = 
                    prev.name, prev.target, prev.target_color, prev.target_idx, prev.is_item, prev.time
                restored = true
            end
        end
        if not restored then cast_state.name = ''; cast_state.queued_time = 0; history_count = 0 end
    end
end)
...
ashita.events.register('command', 'targetbar_cmd', function(e)
    ...
    elseif sub == 'lock'     then cfg.locked = not cfg.locked; settings.save(); print('[targetbar] lock: ' .. tostring(cfg.locked))
    ...
    elseif sub == 'width' and args[3] then cfg.bar_width = tonumber(args[3]) or cfg.bar_width; settings.save(); print('[targetbar] width: ' .. cfg.bar_width)
    elseif sub == 'height' and args[3] then cfg.bar_height = tonumber(args[3]) or cfg.bar_height; settings.save(); print('[targetbar] height: ' .. cfg.bar_height)
