...
-- cast_history bounded to prevent unbounded growth
local cast_history    = {}
...
local function refresh_party_cache(now)
    ...
    local ok, sid = pcall(function() return party:GetMemberServerId(0) end)
    ...
    for i = 0, 17 do
        ...
        if active_ok and active ~= 0 then
            local id_ok, sId = pcall(function() return party:GetMemberServerId(i) end)
...
local function draw_bar(data, win_id, pos_x, pos_y, bar_h, is_sub, spell_name)
    local flags = (cfg.locked or is_sub) and WIN_FLAGS_LOCKED or WIN_FLAGS
...
ashita.events.register('packet_out', 'targetbar_packet_out', function(e)
    ...
    if target_idx ~= 0 and entity then
        ...
        local ok_name, tname = pcall(function() return entity:GetName(target_idx) end)
        target_name  = (ok_name and tname) or 'Unknown'
    ...
    -- bounded ring buffer for cast history
    history_count = (history_count % HISTORY_MAX) + 1
...
ashita.events.register('d3d_present', 'targetbar_render', function()
    ...
    if not cb then
        local ok, ncb = pcall(function() return mm:GetCastBar() end)
        if ok and ncb then
            castbar_cache = ncb
            cb = ncb
        end
    ...
    local ok_main, main_idx = pcall(function() return targ:GetTargetIndex(0) end)
    main_idx = ok_main and main_idx or 0

    local sub_active_raw = nil
    local ok_sub, sval = pcall(function() return targ:GetIsSubTargetActive() end)
    if ok_sub then sub_active_raw = sval end
    local is_sub_active = (sub_active_raw ~= nil and sub_active_raw ~= 0 and sub_active_raw ~= false)

    local sub_idx = 0
    if is_sub_active then
        local ok_si, si = pcall(function() return targ:GetTargetIndex(1) end)
        sub_idx = ok_si and si or 0
    end
    ...
    or (sub_idx  ~= last_sub_idx) then
        ...
        sub_data  = (sub_idx ~= 0 and sub_idx ~= main_idx)
                    and parse_target_data(sub_idx, sub_target_cache, true) or nil
    end
